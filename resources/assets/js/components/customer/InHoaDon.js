...
<div className="GOK0K1ECPBC">
      ...
      <div className="gwt-Label">In lúc {ngayhientai}</div>
        </div>
        <div style={{display: 'flex', flexFlow: 'row nowrap', borderBottom: '4px solid #ddd'}}>
          <div style={{marginRight: '10px', alignSelf: 'flex-end', whiteSpace: 'nowrap'}}>
            ...
            <div style={{fontStyle: 'italic', margin: '5px 0'}}><span style={{fontWeight: 600}}>Ngày lập</span>: <span>{ngayhientai}</span></div>
          </div>
        ...
        <div className="GOK0K1ECECC" style={{marginTop: '15px'}}>
          <div style={{flexBasis: '300px'}}>
            <div style={{fontWeight: 600, marginRight: '5px'}}>Khách hàng: {datakhachhang.hoten}</div>
...
{cd.giamgia.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")}
 ...
 <div className="ProcedureItemView-doctor">{cd.saugiam.toString().replace(/\B(?<!\.\d*)(?=(\d{3})+(?!\d))/g, ".")}</div>
